 ...
 * Admin Interface
 *
 * Settings page and post meta boxes for OpenBotAuth configuration.
 *
 * @package OpenBotAuth
 * @since 0.1.0
 */
...
class Admin {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_openbotauth_save_policy', array( $this, 'ajax_save_policy' ) );

		// Clean up old analytics data (older than 30 days).
		add_action( 'admin_init', array( Analytics::class, 'cleanup_old_stats' ) );
	}
	...
	public function register_settings() {
		...
		register_setting(
			...
			array(
				'type'              => 'boolean',
				'default'           => false,
				'sanitize_callback' => array( $this, 'sanitize_use_hosted_verifier' ),
			)
		);
		register_setting(
			'openbotauth_config',
			...
			array(
				'sanitize_callback' => array( $this, 'sanitize_verifier_url' ),
			)
		);
		register_setting(
			'openbotauth_config',
		...
		register_setting(
			...
			array(
				'sanitize_callback' => array( $this, 'sanitize_payment_url' ),
			)
		);
		...
		add_settings_field(
			...
			array( $this, 'render_verifier_url_field' ),
			'openbotauth_config',
			'openbotauth_general'
		);
		...
		add_settings_field(
			...
			array( $this, 'render_teaser_words_field' ),
			'openbotauth_config',
			'openbotauth_policy'
		);
		...
		register_setting(
			...
			array(
				'type'              => 'boolean',
				'default'           => true,
				'sanitize_callback' => array( $this, 'sanitize_ai_boolean' ),
			)
		);
		register_setting(
			'openbotauth_ai',
		...
		register_setting(
			...
			array(
				'type'              => 'integer',
				'default'           => 100,
				'sanitize_callback' => array( $this, 'sanitize_feed_limit' ),
			)
		);
		register_setting(
			'openbotauth_ai',
			...
			array(
				...
				'sanitize_callback' => array( $this, 'sanitize_feed_post_types' ),
			)
		);
	 ...
	 *
	 * @param string $value The submitted URL value.
	...
	public function sanitize_ai_boolean( $value ) {
		...
		if ( ! isset( $_POST['openbotauth_ai_tab_submitted'] ) ) {
			...
			if ( preg_match( '/sanitize_option_(.+)/', $current_filter, $matches ) ) {
				...
				if ( in_array( $option_name, array( 'openbotauth_llms_enabled', 'openbotauth_feed_enabled' ), true ) ) {
					return true;
				}
				return false;
			}
	 ...
	 *
	 * @param mixed $value The submitted value.
	 * @return int The sanitized feed limit.
	 */
	public function sanitize_feed_limit( $value ) {
		...
		if ( ! isset( $_POST['openbotauth_ai_tab_submitted'] ) ) {
			...
			return false !== $existing ? $existing : 100;
		}
	...
	public function sanitize_feed_post_types( $value ) {
		...
		if ( ! isset( $_POST['openbotauth_ai_tab_submitted'] ) ) {
			...
			return false !== $existing ? $existing : array( 'post', 'page' );
		}
							...
							<pre style="background: #f5f5f5; padding: 15px; overflow: auto; margin-top: 10px; font-size: 12px;">
{
	...
	"rate_limit": {
		...
		"window_seconds": 3600
			...
			<?php elseif ( 'ai-artifacts' === $current_tab ) : ?>
				<!-- AI Artifacts Tab -->
	...
	private function render_observed_bots_table() {
		...
		<div class="openbotauth-table-section" style="margin-bottom: 24px;">
			...
			<table class="widefat striped" style="border: none;">
				...
				<tbody>
					<?php
					foreach ( $active_bots as $bot_id => $bot ) :
	...
	private function render_ai_artifacts_section() {
		...
		$feed_post_types = get_option( 'openbotauth_feed_post_types', array( 'post', 'page' ) );
							...
							<td>
								...
								<label style="display: block; margin-bottom: 6px;">
									<input type="checkbox" 
											...
											<?php checked( in_array( $post_type->name, $feed_post_types, true ) ); ?>>
	 ...
	 *
	 * @param string $value The policy JSON value to sanitize.
	 * @return string Sanitized policy JSON.
	 */
	public function sanitize_policy( $value ) {
		...
		if ( isset( $_POST['openbotauth_default_effect'] ) || isset( $_POST['openbotauth_teaser_words'] ) ) {
			$decoded_policy = json_decode( $value, true );
			$policy         = is_array( $decoded_policy ) ? $decoded_policy : array();
	...
	public function render_meta_box( $post ) {
		...
		$policy = $policy ? json_decode( $policy, true ) : array();
	...
	public function ajax_save_policy() {
		...
		// Sanitize the decoded policy structure to prevent header injection and XSS.
		$sanitized = $this->sanitize_policy_array( $decoded );

		// Re-encode the sanitized data for storage.
		$policy = wp_json_encode( $sanitized );
	 ...
	 *
	 * Sanitizes all policy values to prevent header injection and XSS.
	 * Effect values are validated against allowed list.
	 * Integer fields are sanitized with absint().
	 * URL arrays are sanitized with esc_url_raw().
	 *
		...
		foreach ( $policy as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				...
				if ( in_array( $key, array( 'whitelist', 'blacklist' ), true ) ) {
					...
					// Recursively sanitize nested arrays (like 'default', 'rate_limit').
					$sanitized[ $key ] = $this->sanitize_policy_array( $value );
				}
			} elseif ( 'effect' === $key ) {
				// Validate effect is one of allowed values to prevent header injection.
				$sanitized[ $key ] = in_array( $value, $valid_effects, true ) ? $value : 'allow';
