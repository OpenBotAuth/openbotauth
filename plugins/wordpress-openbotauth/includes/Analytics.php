...
class Analytics {
	 ...
	 */
	const DECISION_TYPES = array( 'allow', 'teaser', 'deny', 'pay', 'rate_limit' );
	...
	public static function get_stats( $days = 7 ) {
		...
		for ( $i = 0; $days > $i; $i++ ) {
			...
			$option_name = self::OPTION_PREFIX . $date;
			$day_stats   = get_option( $option_name, array() );
	...
	public static function incrementMeta( $key ) {
		$valid_keys = array( 'signed_total', 'verified_total' );
	...
	public static function incrementBotStat( string $bot_id, string $key ): void {
		$valid_keys = array( 'requests_total', 'signed_total', 'verified_total' );
		if ( ! in_array( $key, $valid_keys, true ) ) {
			return;
		}
	 ...
	 */
	const KNOWN_REF_SOURCES = array( 'chatgpt', 'perplexity', 'claude', 'gemini', 'copilot' );
	...
	public static function cleanup_old_stats() {
		...
		$prefixes = array( self::OPTION_PREFIX, self::META_STATS_PREFIX, self::BOT_STATS_PREFIX, self::REF_STATS_PREFIX );
